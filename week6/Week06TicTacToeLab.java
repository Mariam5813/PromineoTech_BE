...
public class Week06TicTacToeLab {
	...
	public static void main(String[] args) {
		...
		System.out.println("---------------------------------");
		System.out.println("---Welcom to Tic Tac Toe game ---");
		System.out.println("---------------------------------"); 
		while(winner.equals("")&& allSquares<=9) {
			System.out.println(player+"'s turn - enter a number to designated a square for "+player +" in: "); 
			String readInput= in.nextLine();
			squarenum=Integer.valueOf(readInput);
			if (squarenum>0 && squarenum<=9) {
				if (board.isValidSetSquare(squarenum, readInput, player)) {
					if(player.equals("X")) {
						player="O";
						} else {
							player="X";
