...
public class Week05OOPLab {
	...
	public static void main(String[] args) {
		...
		Card card = new Card("Two","Spade",2);
	...
	public static Map<String, List<Card>> createGame(int numOfPlayers){
		...
		System.out.println("The new deck has "+deck.cards.size()+" cards");
		// create a map with player's name and empty list
		for (int i = 1; i <= numOfPlayers; i++) {
		...
		for (int i = 0; i < decksize/numOfPlayers; i++) {
		...
		if (decksize % numOfPlayers != 0) {
			System.out.print("With " + numOfPlayers + " players -- ");
			System.out.println("Cards left in deck: " + decksize%numOfPlayers); 
		} else {
